...
// SurveyRecords field keys to display in the voter detail modal
const VOTER_KEYS = [
  'voterid', 'houseNumber', 'contactNumber', 'dob', 'age',
  'gender', 'religion', 'community', 'economicStatus',
  'employmentStatus', 'employmentType', 'healthStatus', 'homeType',
  'maritalStatus', 'education', 'educationtype', 'differentlyAbled',
];

// Human-readable labels for camelCase keys
const KEY_LABEL = {
  voterid:          'Voter ID',
...
/** Returns the display name for a SurveyRecords voter document */
function voterName(v) {
  const first  = v.firstName  || '';
  const middle = v.middleName || '';
...
export default function SchemeVoters() {
  ...
  const filtered = voters.filter(v => {
    ...
    return voterName(v).toLowerCase().includes(q)
      ...
      <div className="page-inner" style={{ maxWidth: 1100 }}>
        ...
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <p>{search ? 'Try a different search term' : `No survey records for ward ${wardName}`}</p>
          </div>
        ) : (
          <div className="grid-auto stagger">
            {filtered.map((voter, i) => (
              <div key={voter.voterid || voter._id || i} className="card voter-card" onClick={() => openVoter(voter)}>
                ...
                <div className="voter-info">
                  <div className="voter-name">{voterName(voter)}</div>
          ...
          <div className="modal">
            <div className="modal-header">
              <div className="flex items-center gap-16">
                <div style={{ width: 50, height: 50, borderRadius: 14, background: 'linear-gradient(135deg,var(--gold),var(--cyan))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 22, color: '#090e1c', flexShrink: 0 }}>
                  {(modal.firstName || '?')[0].toUpperCase()}
                </div>
                <div>
                  <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 20, fontWeight: 800 }}>{voterName(modal)}</h2>
            ...
            <div className="modal-body">
              ...
              <div className="detail-grid mb-20">
                {VOTER_KEYS.filter(k => modal[k] != null && modal[k] !== '').map(k => (
                  <div key={k} className="detail-item">
                    <div className="detail-key">{KEY_LABEL[k] || k}</div>
                    <div className="detail-val">{String(modal[k])}</div>
                  </div>
                ))}
              </div>
